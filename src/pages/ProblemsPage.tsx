...
const ProblemsPage: React.FC = () => {
  ...
  // Get unique topics for filter
  const uniqueTopics = useMemo(() => Array.from(new Set(problems.map(p => p.topic))).filter(Boolean), [problems]);
  ...
  const filteredProblems = useMemo(() => 
    ...
    [problems, searchTerm, filterDifficulty, filterStatus, filterTopic]
  );
  ...
  const handleProblemClick = useCallback(async (problem: any) => {
    setIsDetailLoading(true);
    try {
      // Pre-fetch similar problems as a proxy (adjust if ProblemDetail has its own fetch)
      await getSimilarProblems(problem.id);
      setSelectedProblem(problem);
      setEditMode(false);
    } catch (error) {
      console.error('Error preparing problem detail:', error);
    } finally {
